...
|---|---|---|---|
| Пароли | **Argon2id** (m=64MB, t=3, p=1), соль на юзера | PBKDF2-SHA256, 200k, соль | апгрейд PBKDF2 → Argon2id |
...
| Refresh tokens | шифровать (envelope/KMS), привязка к устройству, ротация с детекцией повторного использования | — | — |
| Cookies (если веб) | `HttpOnly; Secure; SameSite=Strict`, подпись, короткий TTL | API на Bearer | при веб-дашборде |
| SSH-ключи | **только на устройстве**, OS keychain; сервер — никогда | не хранятся на сервере | держать так |
| Память ассистента | E2E (§4); минимум — AES-256-GCM на сервере под KMS-ключом | SQLite/JSON в открытом виде | шифрование at-rest → E2E |
...
- Пароли и все секреты — **только хэши** (PBKDF2 + соль; токены/ключи — SHA-256).
...
1. **TLS/HTTPS** — домен + Caddy/Let’s Encrypt (или PaaS-домен). Закрывает главный текущий риск (открытый http).
2. **Argon2id** для паролей (миграция «при следующем входе», без сброса).
3. **Шифрование at-rest** чувствительных таблиц (память, документы, чаты) — AES-256-GCM, ключ из секрет-менеджера.
...
> Пункты 1–2 — быстрые и дают наибольший прирост безопасности на единицу усилий.
> 3–4 — средний срок. 5–6 — целевое состояние для «миллионов пользователей».
