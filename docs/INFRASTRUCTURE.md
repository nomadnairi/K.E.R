...
сервера (`2.26.80.6`, если это по-прежнему тот же VPS):

| Тип | Имя | Значение     | TTL   |
|-----|-----|--------------|-------|
| A   | @   | 2.26.80.6    | 3600  |
| A   | www | 2.26.80.6    | 3600  |

Проверка распространения DNS перед запуском `init-letsencrypt.sh`:

```bash
dig +short ваш-домен.ru
# должно вернуть 2.26.80.6
```
...
Существующий десктоп-клиент K.E.R. зашит на `http://2.26.80.6:8000` напрямую
