...
# 2. API живёт под /api/
curl -s https://ker-ai.online/api/status
# ждём: {"status":"operational", ...}
...
**`/api/status` отвечает 502 Bad Gateway**
...
1. **Шрифт Inter не приложен.** В стилях он указан первым, но файлы не
...
2. **Конфиг nginx не проверен командой `nginx -t`** в моём окружении —
   там не было запущенного Docker. Синтаксис я вычитал, но первый запуск
   стоит делать, глядя в `logs nginx`.

3. **Веб-кабинет — заглушка.** Бэкенд под него готов полностью (вход по
   коду, устройства, память, тарифы, автоматизации), интерфейс — следующая
   задача.

4. **Цена в тарифах** взята из кода: Plus = 2500 Telegram Stars, Pro = 8000.
